...
describe(@"Percent Encoding", ^{
    ...
    it(@"encodes an extended set of illegal url characters", ^{
        NSString *encodedString = [decodedTestString DLC_stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        expect(encodedString).to.equal(encodedTestString);
    });
    
    it(@"does not encode a string without illegal characters", ^{
        NSString *encodedString = [plainTestString DLC_stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
describe(@"Percent Decoding", ^{
    ...
    it(@"decodes a string with escaped url characters", ^{
        NSString *decodedString = [encodedTestString DLC_stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        expect(decodedString).to.equal(decodedTestString);
    });
    
    it(@"does not decode a string without escaped characters", ^{
        NSString *decodedString = [plainTestString DLC_stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
describe(@"Dictionary to Query String", ^{
    ...
    it(@"should construct a query string from a dictionary", ^{
        ...
        NSString *query = [NSString DLC_queryStringWithParameters:params];
...
describe(@"Query String to Dictionary", ^{
    ...
    it(@"should parse a valid query string into a dictionary", ^{
        ...
        NSDictionary *params = [query DLC_parametersFromQueryString];
