...
describe(@"Percent Encoding", ^{
    ...
    it(@"encodes an extended set of illegal url characters", ^{
        NSString *encodedString = [decodedTestString BTN_stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        expect(encodedString).to.equal(encodedTestString);
    });
    
    it(@"does not encode a string without illegal characters", ^{
        NSString *encodedString = [plainTestString BTN_stringByAddingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
...
describe(@"Percent Decoding", ^{
    ...
    it(@"decodes a string with escaped url characters", ^{
        NSString *decodedString = [encodedTestString BTN_stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
        expect(decodedString).to.equal(decodedTestString);
    });
    
    it(@"does not decode a string without escaped characters", ^{
        NSString *decodedString = [plainTestString BTN_stringByReplacingPercentEscapesUsingEncoding:NSUTF8StringEncoding];
