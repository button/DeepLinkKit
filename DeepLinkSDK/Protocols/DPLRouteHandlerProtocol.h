...
/**
 ...
 a bit more work resulting a mess of code in your block handler. Implementing the `DPLRouteHandlerProtocol' helps move that complexity from route registration to a more sensible place.
 
 @note `DPLRouteHandlerProtocol' have two rules, they MUST specify a target view controller
 and they must conform to the `DPLTargetViewController' protocol. You can provide a view controller 
 by defining `targetViewController:'.
 
 @see DPLTargetViewController
 */
@protocol DPLRouteHandlerProtocol <NSObject>
...
/**
 ...
 @note This method should only be implemented if you're looking for maximum flexibility in
